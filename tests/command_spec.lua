...
describe(":Occurrence command", function()
  ...
  describe("api data tables", function()
    it("exposes require('occurrence').operators / .actions as tables", function()
      local o = require("occurrence")
      assert.equals("table", type(o.operators), "operators should be the config table, not an apply stub")
      assert.equals("table", type(o.actions), "actions should be the config table, not an apply stub")
      assert.are.equal(require("occurrence.api").operators, o.operators)
      assert.are.equal(require("occurrence.api").actions, o.actions)
    end)

    it("does not register `operators` / `actions` as subcommands", function()
      local err = stub(require("occurrence.log"), "error")
      vim.cmd("Occurrence operators")
      vim.cmd("Occurrence actions")
      assert.spy(err).was_called(2) -- reported as unknown, not a crash
      err:revert()
    end)

    it("still runs a real subcommand", function()
      bufnr = util.buffer({ "foo bar foo" })
      vim.api.nvim_win_set_cursor(0, { 1, 0 })
      vim.cmd("Occurrence mark")
      assert.equals(2, #vim.api.nvim_buf_get_extmarks(bufnr, MARK_NS, 0, -1, {}))
    end)
  end)
end)
