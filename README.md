...
If the key sequence will operate when in visual mode, it can be defined as just the sequence:

```lua
require("occurrence").setup({
  operators = {
    ["~"] = {
      desc = "Swap case using ~",
      operator = "~",
    }
  }
})
...
Otherwise, you can define the full command to operate on the selection:

```lua
require("occurrence").setup({
  operators = {
    ["g@q"] = {
      desc = "Apply @q macro to each occurrence",
      operator = ":'<'>normal @q<CR>",
    }
  }
})
```
...
The operator function receives two parameters:
...
```lua
-- Return a string to replace the mark
operator = function(mark, ctx)
  return "replacement"
end

-- Return array of strings for multi-line replacement
operator = function(mark, ctx)
...
operator = function(mark, ctx)
  if some_condition then
    return false -- Stop processing all marks
  end
  return "replacement"
end
```
...
For operations requiring asynchronous work (LSP requests, external commands, user input), return a function that accepts a `done` callback:
...
require("occurrence").setup({
  operators = {
    format_with_prettier = {
          ...
          vim.system(
            ...
            function(result)
              if result.code == 0 then
                -- Call done with replacement text
                done(vim.split(result.stdout, "\n"))
              else
      ...
      batch_size = 5, -- Optional: limit concurrent operations (default: 10)
    },
  }
})
```
...
The `before` function receives two parameters:

- `marks`: Array of all marks being processed
- `ctx`: Operator Context (same as for `operator` function)

and can return:

- `nil` or `true` to proceed with the operation
- `false` to cancel the operation
- or a function that accepts `done` callback for async setup
- when async, call `done()` when setup is complete, or `done(false)` to cancel

A common pattern is to store setup results in the `ctx` object for access in the `operator` function.

For example, prompting the user for a transformation choice before processing marks:

```lua
require("occurrence").setup({
  operators = {
    ["g~"] = {
      desc = "Transform with user choice",
      before = function(marks, ctx)
        -- Async setup - prompt user
        return function(done)
          vim.ui.select({ "uppercase", "lowercase", "reverse" }, {
            prompt = "Choose transformation:",
          }, function(choice)
            if not choice then
              done(false) -- Cancel if user cancels
            else
              ctx.user_choice = choice -- Store in context
              done() -- Signal setup complete
            end
          end)
        end
      end,
      operator = function(mark, ctx)
        -- Access user's choice from context
        local choice = ctx.user_choice
        local text = table.concat(mark.text, "\n")
